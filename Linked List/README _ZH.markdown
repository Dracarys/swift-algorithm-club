...
> **Note:** 为了避免循环持有，这里我们将 `previous` 声明为弱引用。假设有一个节点 `A` 其后跟着一个节点 `B`, 此时 `A` 指向 `B` 但 `B` 也同样指向 `A`。这就出现了循环持有的情况，即使你删除了这个两个节点，由于他们互相持有也将持续存活（于内存中）。这不是我们期望的，所以我们将其中的一个指针设为 `weak` 以打破这种循环。
